  ...
  Select.prototype = {
    ...
    create: function () {
      ...
      this.select = $(
        '<div id="' +
          this.id +
          '" class="ui-select"><ul class="ui-select-content"></ul></div>'
      );
      ...
      if (!this.settings.model && this.model.length == 0) {
        ...
        for (var i = 0, l = _this.model.length; i < l; i++) {
          ...
          _html +=
            ...
            dis +
            ' value="' +
            _this.escape(item.value) +
            '">' +
            _this.escape(item.text) +
            "</option>";
        }
    ...
    escape: function (v) {
      return v.toString().replace(/\'/gim, "&apos;").replace(/\"/gim, "&quot;");
    },
    // 设置select值
    setValue: function (val, txt) {
      ...
      this.selectContent.find("li").each(function () {
        if ($(this).data("value") == val) {
          ...
          $(this)
            .addClass("ui-select-item-selected")
            .siblings()
            .removeClass("ui-select-item-selected");
          // 如果当前列表选项的value值等于设置的值，则不触发change事件。否则需要trigger改变事件，进行更新
          if (_this.value != val) {
            _this.settings.selected &&
              _this.settings.selected.call(_this, val, txt);
